...
def training_args():
    ...
    # -- model
    parser.add_argument("--in_size", type=tuple, default=(112, 96))  # FIXED
    ...
    parser.add_argument(
        ...
        type=str,
        default="arcface",
        choices=["softmax", "sphere", "cosface", "arcface", "mvcos", "mvarc"],
    )
    parser.add_argument(
        "--hard_mode", type=str, default="adaptive", choices=["fixed", "adaptive"]
    )  # MV
    parser.add_argument(
        "--loss_mode",
        type=str,
        default="ce",
        choices=["ce", "focal_loss", "hardmining"],
    )
    ...
    )  # TODO : [0.1 for backbone]
    parser.add_argument(
        "--lr_adjust", type=list, default=[2, 3, 4]
    )  # TODO : [16, 25, 35]
    ...
    parser.add_argument(
        "--lfw_dir", type=str, default=osp.join(lfw_dir, "align_112_112")
    )
    parser.add_argument(
        "--train_file",
        type=str,
        default=osp.join(osp.join(root_dir, "ldmark_casia_mtcnncaffe.txt")),
    )
    parser.add_argument(
        "--pairs_file", type=str, default=osp.join(lfw_dir, "anno_file/pairs.txt")
    )
    ...
    parser.add_argument("--save_to", type=str, default="pfe")
    parser.add_argument(
