...
class Trainer(TrainerBase):
    def _model_loader(self):
        ...
        if "criterion" in self.model_args.backbone:
            warnings.warn("Unused criterion of the backbone")

        self.head = None
        self.scale_predictor = None

        if self.model_args.scale_predictor:
            self.scale_predictor = mlib.heads[self.model_args.scale_predictor.name](
                **utils.pop_element(self.model_args.scale_predictor, "name"),
            )

        self.criterion = mlib.criterions_dict[
            self.model_args.criterion.name](
        ...
        if self.args.resume:
            ...
            self.scale_predictor.load_state_dict(model_dict["scale"])
        ...
        self.partial_fc = PartialFC(
            rank=self.rank, local_rank=self.local_rank, world_size=self.world_size,
            batch_size=self.model_args.batch_size, margin_softmax=self.criterion,
            num_classes=self.model_args.num_classes, resume=self.model_args.resume,
            sample_rate=self.model_args.sample_rate, embedding_size=self.model_args.embedding_size,
            prefix=self.checkpoints_path)
        ...
        if self.model_args.backbone.learnable:
            learnable_parameters.extend(list(self.backbone.parameters()))
        if self.model_args.scale_predictor.learnable:
            learnable_parameters.extend(list(self.scale_predictor.parameters()))
