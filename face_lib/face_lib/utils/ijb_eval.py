...
def extract_gallery_prob_data(data_path, subset, save_path=None, force_reload=False):
    ...
    print(
        ...
        % (
            gallery_templates.shape,
            gallery_subject_ids.shape,
            np.unique(gallery_templates).shape,
        )
    )
...
def verification_11(
    template_norm_feats=None, unique_templates=None, p1=None, p2=None, batch_size=100000
):
...
def evaluation_1N(query_feats, gallery_feats, query_ids, reg_ids):
    ...
    for query_id in query_ids:
        ...
        if len(pos) != 1:
            raise RuntimeError(
                "RegIdsError with id = {}， duplicate = {} ".format(query_id, len(pos))
            )
        mask.append(pos[0])
    ...
    print(
        ...
        % (
            correct_num_1 / query_num,
            correct_num_5 / query_num,
            correct_num_10 / query_num,
        )
    )
    ...
    print(
        "pos_sims: %s, neg_sims: %s, neg_sims_sorted: %d"
        % (pos_sims.shape, neg_sims.shape, len(neg_sims_sorted))
    )
...
class IJB_test:
    ...
    def run_model_test_1N(self):
        ...
        (
            gallery_templates_feature,
            gallery_unique_templates,
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Saved model file, could be keras h5 / pytorch jit pth / mxnet",
    )
    parser.add_argument(
        "-d", "--data_path", type=str, default="./", help="Dataset path"
    )
    parser.add_argument(
        "-s",
        "--subset",
    ...
    parser.add_argument(
        "-b", "--batch_size", type=int, default=64, help="Batch size for get_embeddings"
    )
    parser.add_argument(
        "-R",
        "--save_result",
    ...
    if args.plot_only != None and len(args.plot_only) != 0:
        ...
        if args.save_result == default_save_result_name:
            args.save_result = default_save_result_name.format(
                model_name=model_name, subset=args.subset
            )
    return args
