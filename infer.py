...
class ProbFace(object):
    ...
    def _model_loader(self):
        ...
        self.model["backbone"] = mlib.MobileFace(
            self.args.in_feats, self.args.drop_ratio
        )
        ...
        if self.device and len(self.args.gpu_ids) > 1:
            self.model["backbone"] = torch.nn.DataParallel(
                self.model["backbone"], device_ids=self.args.gpu_ids
            )
            self.model["uncertain"] = torch.nn.DataParallel(
                self.model["uncertain"], device_ids=self.args.gpu_ids
            )
    ...
    def cal_pair_mls(mu1, mu2, logsig_sq1=None, logsig_sq2=None):
        ...
        mu_diff = mu1 - mu2
        mls_pointwise = torch.mul(mu_diff, mu_diff) / sig_sq_mutual + torch.log(
            sig_sq_mutual
        )
...
def infer_args():
    ...
    parser.add_argument(
        "--resume", type=str, default=os.path.join(cp_dir, "pfe/sota.pth.tar")
    )  # checkpoint
...
if __name__ == "__main__":
    ...
    mls_score1 = probu._process_pair(face2, face3)
    mls_score2 = probu._process_pair(face1, face2)
    print(mls_score1, mls_score2)
